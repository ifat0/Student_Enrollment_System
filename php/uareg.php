            ...
            <div class="container">
                <div class="signup-content">
                    <div class="signup-form">
                        <h3 class="form-title">University Authority Registration</h3>
                        <form action="uareg.php" method="POST" class="register-form" id="register-form">
                            <div class="form-group">
                                <label for="name"><i class="zmdi zmdi-account material-icons-name"></i></label>
                                <input type="number" name="sid" id="sid" placeholder="Enter Authority ID" />
                            </div>
                            <div class="form-group">
                                <label for="name"><i class="zmdi zmdi-account material-icons-name"></i></label>
                                <input type="text" name="name" id="name" placeholder="Enter Authority's Name" />
                            </div>
                            <div class="form-group">
...
if(isset($_POST['email'])){
    $sql=$data->getData("select email from student where email='".$_POST['email']."' ");
    ...
    if($rows==null){
        ...
        if($_POST['pass']==$_POST['re_pass']){
            ...
            // if (!empty($_POST['name']) && !empty( $_POST['phone']) && !empty($_POST['address']) && !empty($_POST['email']) && !empty($_POST['pass'])){
                ...
                $database->connect($sql="Insert into universityauthority(ua_Id, ua_Name, ua_Email, ua_Phone, ua_address, ua_pass) values('".$_POST['sid']."','".$_POST['name']."','".$_POST['email']."','".$_POST['phone']."','".$_POST['address']."','".$_POST['pass']."')");
